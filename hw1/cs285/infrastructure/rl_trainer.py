...
class RL_Trainer(object):
    ...
    def __init__(self, params):
        ...
        self.agent.actor.sess.run(tf.global_variables_initializer())
    ...
    def collect_training_trajectories(self, itr, load_initial_expertdata, collect_policy, batch_size):
        ...
        # TODO: bug here, not load data
        if itr == 0:
            with open(load_initial_expertdata, 'rb') as f:
                data = pickle.loads(f.read())
            # print("expert data : ",data)
            return data,0,None

        # TODO collect data to be used for training--remain bugs
    ...
    def train_agent(self):
        ...
        for train_step in range(self.params['num_agent_train_steps_per_iter']):
            ...
            losses = self.agent.actor.sess.run(self.agent.actor.loss,
                                               feed_dict={self.agent.actor.observations_pl: ob_batch,
                                                          self.agent.actor.acs_labels_na: ac_batch})
            print("the loss of step %d : %f"%(train_step,losses))
    ...
    def do_relabel_with_expert(self, expert_policy, paths):
        ...
        for i in range(len(paths)):
            paths[i]['actions'] = expert_policy.get_action(paths[i]['observation'])
